...
<section class="wa-ingredients" aria-label="원료와 기술">
  <style>
    ...
    .wa-hero{background:linear-gradient(160deg,#0E2647 0%,#123A73 55%,#0E4FB0 100%);
      border-radius:22px;padding:clamp(28px,4vw,52px);color:#fff;overflow:hidden;position:relative}
    .wa-hero::after{content:"";position:absolute;inset:0;
    ...
    .wa-card{background:var(--card);border:1px solid var(--line);border-radius:18px;
      padding:26px 24px;display:flex;flex-direction:column;transition:transform .2s,box-shadow .2s;
      border-top:3px solid var(--cobalt)}
    .wa-card:hover{transform:translateY(-3px);box-shadow:0 14px 34px rgba(16,35,63,.08)}
    ...
    .wa-delivery{margin-top:20px;background:var(--cobalt-soft);border:1px solid #D3E2FC;
      border-radius:18px;padding:26px 28px;display:flex;gap:22px;align-items:flex-start;flex-wrap:wrap}
    ...
    .wa-delivery-body{flex:1;min-width:240px}
    .wa-delivery h4{font-family:var(--serif);font-weight:700;font-size:19px}
    .wa-delivery h4 sup{font-size:.5em;top:-.8em}
    .wa-delivery p{margin-top:8px;font-size:14px;line-height:1.75;color:var(--slate)}
    .wa-delivery p b{color:var(--ink);font-weight:600}
    ...
    @media(max-width:860px){
      .wa-paths{grid-template-columns:1fr}
  ...
  <div class="wa-wrap">
    ...
    <div class="wa-grid">
      ...
      <div class="wa-card">
        ...
        <p class="wa-c-desc">아유르베다와 전통 한의학에서 수백 년간 존중받아온 유기농 아답토젠 — <b>영지·라이언스메인·동충하초·마카·인삼</b>·황기 — 몸이 스트레스에 무리 없이 균형 있게 대응하도록 돕기 위해 선택했습니다. 자체 항산화·미네랄·아미노산도 함께 담깁니다. 아답토젠은 그 토양만큼만 깨끗합니다.</p>
      </div>
...
const IngredientsTechnology = () => (
  <div className="min-h-screen bg-background">
    <Header />
    <div className="pt-24" dangerouslySetInnerHTML={{ __html: sectionHtml }} />
    <Footer />
    <FloatingCTA />
  </div>
);
